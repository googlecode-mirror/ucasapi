...
class Prioridad extends CI_Controller{
	...
	function index(){
		...
		if($idRol != ""){//Si está en sesión
			...
			if($allowedPage){//Si el usuario según rol tiene permiso para acceder a la página
				...
				$this->load->view("prioridadView", array("menu"=> $menu, "userName" => $userName, "roleName" => str_replace("%20", " ", $roleName)));//Se agrega el código del menú y el nombre del usuario como variables al view
	...
	function prioridadRead(){
		$this->load->model("prioridadModel");	
		echo json_encode($this->prioridadModel->read());
	}
	
	
	function prioridadAutocompleteRead(){
		$this->load->model("prioridadModel");
		
		$autocompleteData = $this->prioridadModel->autocompleteRead();		
		
		echo json_encode($autocompleteData);
	}
	
	function prioridadDelete(){
		$this->load->model("prioridadModel");
		
		$deleteInfo = $this->prioridadModel->delete();		
		
		echo json_encode($deleteInfo);
	}


	function prioridadValidateAndSave(){
		$this->load->model("prioridadModel");
		$retArray = array();
		
		$validationInfo = $this->prioridadModel->saveValidation();
		...
		if($validationInfo["status"] == 0){//Los datos ingresados pasaron las validaciones
			...
			if($accionActual == ""){//Si no se recibe el id, los datos se guardarán como un nuevo registro
				$retArray = $this->prioridadModel->create();
			}
			else{
				$retArray = $this->prioridadModel->update();
			}
