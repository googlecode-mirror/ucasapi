	...
	function index(){
		...
		if($idRol != ""){//Si est� en sesi�n
			...
			if($allowedPage){//Si el usuario seg�n rol tiene permiso para acceder a la p�gina
				...
				$this->load->view("ActividadaView", array("menu"=> $menu, "userName" => $userName, "roleName" => str_replace("%20", " ", $roleName)));//Se agrega el c�digo del men� y el nombre del usuario como variables al view
		...
		$this->load->view("actividadaView", array("menu"=> $menu, "idUsuario" => $idUsuario,"userName" => $userName, "roleName" => str_replace("%20", " ", $roleName)));//Se agrega el c�digo del men� y el nombre del usuario como variables al view
